...
public:
  static const int REMOVE_SUCCESS = 0,
                   REMOVE_DONE = 1,
                   REMOVE_NOWORK = 2;
  WorkQueue(int num_threads) {
  ...
  int remove(int thread_num, T& item) {
    int retval;
    omp_set_lock(&m_lock);
    if (m_queue.size() > 0) {
      item = m_queue.front();
      ...
      m_pop_ids |= 1 << thread_num;
      if (m_pop_ids == (1 << m_num_threads) - 1) {
        retval = REMOVE_DONE;
      }
      else {
        retval = REMOVE_NOWORK;
      }
    }
...
class BVHAccel : public Aggregate {
 public:
   ...
   */
  BVHAccel(const std::vector<Primitive*>& primitives, size_t numBVHThreads, size_t max_leaf_size = 4);
  ...
  void partition(WorkQueue<PartitionArgs>& work_queue, BVHNode &node, size_t max_leaf_size);
