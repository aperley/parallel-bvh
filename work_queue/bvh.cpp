...
BVHAccel::BVHAccel(const std::vector<Primitive *> &_primitives,
  ...
  omp_set_nested(0);  // no nested parallel calls
  ...
  WorkQueue<PartitionArgs> work_queue(numBVHThreads);

  PartitionArgs rootArgs;
  rootArgs.node = root;
  rootArgs.max_leaf_size = max_leaf_size;
  work_queue.add(rootArgs);

  #pragma omp parallel
  {
    while (true) {
      PartitionArgs args;
      int status = work_queue.remove(omp_get_thread_num(), args);
      if (status == WorkQueue<PartitionArgs>::REMOVE_SUCCESS) {
        partition(work_queue, *(args.node), args.max_leaf_size);
      }
      else if (status == WorkQueue<PartitionArgs>::REMOVE_DONE) {
        break;
      }
    }
  }
...
void BVHAccel::partition(WorkQueue<PartitionArgs>& work_queue, BVHNode& node, size_t max_leaf_size) {
  ...
  PartitionArgs largs, rargs;
  largs.node = node.l;
  largs.max_leaf_size = max_leaf_size;
  work_queue.add(largs);
  rargs.node = node.r;
  rargs.max_leaf_size = max_leaf_size;
  work_queue.add(rargs);
